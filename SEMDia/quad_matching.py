...
from src.utils_for_triplets import MyDataLoader, RelationMetric
from src.model_triplets import BertWordPair
...
class Main:
    ...
    def train_iter(self):
        ...
        for i, data in enumerate(train_data):
            loss, _,_,_ = self.model(self.global_epoch, **data)
            losses.append([w.tolist() for w in loss])
            for i in range(len(losses)):
                if math.isnan(losses[i][0]):
                    losses[i][0] = 0
                if math.isnan(losses[i][1]):
                    losses[i][1] = 0
            ...
            description = "Epoch {}, entity loss:{:.4f}, rel loss: {:.4f}".format(self.global_epoch,
                                                                                  *np.mean(losses, 0))
            # description = "Epoch {}, entity loss:{:.4f}, rel loss: {:.4f}".format(self.global_epoch,loss[0].item(), loss[1].item())
    ...
    def evaluate_iter(self, dataLoader=None):
        ...
        for i, data in enumerate(dataiter):
            with torch.no_grad():
                _, (pred_ent_matrix, pred_rel_matrix),n,n2 = self.model(**data)
    ...
    def test(self):
        ...
        PATH = self.save_path
        PATH = os.path.join(self.config.target_dir, "{}_{}.pth.tar").format(self.config.lang, '3babf41a')#a617e4d3    3babf41a
        ...
        score, _, res, intra, _, pair, entiy = result
        score = intra
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch_size):
            ...
            self.train_iter()
            self.evaluate_iter()
            score, _, res, _, _, pair, entiy = self.relation_metric.compute()
    ...
    def load_param(self):
        ...
        self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=self.config.warmup_steps,
                                                         num_training_steps=10 * self.trainLoader.__len__())
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--other_lr', type=float, default=1e-5)
    parser.add_argument('-c', '--cuda_index', type=int, default=0, help='CUDA index')
    parser.add_argument('-s', '--seed', type=int, default=42, help='random seed')
    parser.add_argument('-p', '--parallel', type=float, default=0.15, help='2D SEM parallel ratio')
    parser.add_argument('-t', '--types', type=int, default=2, help='partitioning type in {0: global, 1: reply relation, 2: 2D SEM}')
