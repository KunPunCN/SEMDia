...
class Template:
    ...
    def post_process(self, line, key='quad'):
        ...
        if key in ['intra', 'inter']:
            for comb in line['triplets']:
                ...
                if key == 'intra' and distance > 0: continue#句子内
                if key == 'inter' and distance == 0: continue#句子间
    ...
    def forward(self, print_line=False):
        ...
        ta,to, ao = scores[3], scores[4], scores[5]
        t, a, o = scores[0], scores[1], scores[2]
        return micro_score, iden_score, res, intra, inter, (ta[2]+ta[2]+ao[2])/3, (t[2]+a[2]+o[2])/3
