    ...
    def collate_fn(self, lst):
        doc_id, input_ids, input_masks, input_segments,sentence_length, token2sents, utterance_index, \
            token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
                triplets, pairs, entity_list, rel_list, speakers, replies, utterance_spans,input_ids2, input_masks2, input_segments2, token2sentid = zip(*lst)
        ...
        # for m in range(len(new_thread_masks)):
        #     for (i,j) in zip(rel_matrix[m], new_thread_masks[m].tolist()):
        #         for (a,b) in zip(i,j):
        #             if a>0 and b==0:
        #                 print(doc_id[m])
        #                 print(utterance_spans[m])
        #                 print(thread_length[m])
        #                 print(rel_lists[m], 'xxxxxxxx')


        #只在上半三角计算
        # new_thread_masks = torch.triu(new_thread_masks, diagonal=1)
        # gs = []
