...
class Preprocessor:
    ...
    def get_neighbor(self, utterance_spans, replies, max_length, speaker_ids, thread_nums):
        ...
        # thread_mask[head_start: head_end + 1, :] = 1
        # thread_mask[:, head_start: head_end + 1] = 1
        # for i, (s, e) in enumerate(thread_spans):
        #     if i == 0: continue
        #     s0 = utterance_spans[s][0]
        #     e0 = utterance_spans[e-1][1]
        #     thread_mask[s0: e0+1, s0: e0+1] = 1
        ...
        for i, (s, e) in enumerate(thread_spans):
            ...
            for j in range(s, e):
                ...
                for k in range(s, e):
                    ...
                    thread_mask[s0 : e0 + 1, s1: e1 + 1] = 1
                    thread_mask[s1 : e1 + 1, s1: e1 + 1] = 1
                    thread_mask[s0 : e0 + 1, s0: e0 + 1] = 1
                    thread_mask[s1 : e1 + 1, s0: e0 + 1] = 1





        return reply_mask.tolist(), speaker_mask.tolist(), thread_mask.tolist(), thread_spans
    ...
    def check_text(self, tokenized_text, source_text):
        if self.config.bert_path.split('/')[-1] in ['roberta-large', 'roberta-base']:
    ...
    def transform2indices(self, dataset, mode='train'):
        ...
        for document in dataset:
            ...
            # tokens2 = [[[self.config.cls] + w + [self.config.sep]] for w in sentences]
            tokens2 = []
            input_masks2 = []
            input_segments2 = []
            max_sentence_len = 0
            for w in sentences:
                x = [self.config.cls] + w + [self.config.sep]
                tokens2.append(x)
                input_masks2.append([1] * len(x))
                input_segments2.append([0] * len(x))
                if len(x) > max_sentence_len:
                    max_sentence_len = len(x)
            for i in range(len(tokens2)):
                if len(tokens2[i])<max_sentence_len:
                    y = max_sentence_len - len(tokens2[i])
                    tokens2[i] = tokens2[i] + [self.config.pad] * y
                    input_masks2[i] = input_masks2[i] + [0] * y
                    input_segments2[i] = input_segments2[i] + [0] * y
            input_ids2 = [self.tokenizer.convert_tokens_to_ids(line) for line in tokens2]
            ...
            reply_mask, speaker_masks, thread_masks, thread_spans = self.get_neighbor(utterance_spans, replies, sum(sentence_length), speakers, thread_nums)
            ...
            # for k in relation_lists:
            #     if thread_masks[k[0]][k[2]] == 0:
            #         print(doc_id)
            #         print(utterance_spans)
            #         print(thread_spans)
            #         print(k, 'xxxxxxxx')
            ...
            res.append((doc_id, input_ids, input_masks, input_segments, sentence_length, nsentence_ids, utterance_index, token_index, 
                        ...
                        new_triplets, pairs, entity_lists, relation_lists, speakers, old_replies, utterance_spans, input_ids2, input_masks2, input_segments2, token2sentid))
        return res
