...
class MyDataLoader:
    def __init__(self, cfg):
        ...
        for i in ['train', 'test', 'valid']:
            res = []
            path2 = os.path.join(cfg.target_dir, '{}_{}.json'.format(cfg.lang, i))
            self.data2 = json.load(open(path2, 'r'))
            for j in range(len(self.data[i])):
                doc_id, input_ids, input_masks, input_segments, sentence_length, token2sents, utterance_index, \
                    ...
                    triplets, pairs, entity_list, rel_list, speakers, replies, utterance_spans, input_ids2, input_masks2, input_segments2, token2sentid = list(self.data[i][j])
                doc_id2, pred_t, pred_a, pred_o = self.data2[j]['doc_id'], self.data2[j]['targets'], self.data2[j]['aspects'], self.data2[j]['opinions']
                assert doc_id == str(doc_id2)
                # print(doc_id,'xxxxxxxxxxxxxx')
                # print(entity_list)
                word2pieces = defaultdict(list)
                for p, w in enumerate(pieces2words):
                    word2pieces[w].append(p)
                targets = [(word2pieces[x][0], word2pieces[y - 1][-1] + 1) for x, y in pred_t]
                aspects = [(word2pieces[x][0], word2pieces[y - 1][-1] + 1) for x, y in pred_a]
                opinions = [(word2pieces[x][0], word2pieces[y - 1][-1] + 1) for x, y in pred_o]
                targets = [(x[0] + token2sentid[x[0]] + 1, x[1] + token2sentid[x[0]]) for x in targets]
                aspects = [(x[0] + token2sentid[x[0]] + 1, x[1] + token2sentid[x[0]]) for x in aspects]
                opinions = [(x[0] + token2sentid[x[0]] + 1, x[1] + token2sentid[x[0]]) for x in opinions]
                # print(targets)
                # print(aspects)
                # print(opinions)
                res.append((doc_id, input_ids, input_masks, input_segments, sentence_length, token2sents, utterance_index, \
                    token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
                    triplets, pairs, entity_list, rel_list, speakers, replies, utterance_spans, input_ids2, input_masks2, input_segments2, token2sentid, targets, aspects, opinions))
            self.data[i] = res
    ...
    def collate_fn(self, lst):
        doc_id, input_ids, input_masks, input_segments, sentence_length, token2sents, utterance_index, \
            token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
            triplets, pairs, entity_list, rel_list, speakers, replies, utterance_spans, input_ids2, input_masks2, input_segments2, token2sentid, pred_t, pred_a, pred_o = zip(
            *lst)
        ...
        # for i in range(len(new_thread_masks)):
        #     for k in triplets[i]:
        #         if new_thread_masks[i, k[0], k[2]] != 1 or new_thread_masks[i, k[0], k[4]] != 1 or new_thread_masks[i, k[2], k[4]] != 1:
        #             print(doc_id)
        #             print(utterance_spans[i])
        #             print(triplets[i], 'xxxxxxxx')

        # gs = []
        # for i in range(len(speakers)):
        #     g = build_hgraph(token2sents[i], speakers[i], replies[i], dialogue_length[i])
        #     gs.append(g)
        hgraphs = 0  # dgl.batch(gs).to(self.config['device'])

        links = [[]for i in range(len(triplets))]
        for i in range(len(triplets)):
            link = []
            triplet = triplets[i]
            for k in range(len(triplet)):
                a0,a1,b0,b1,c0,c1,_ = triplet[k]
                x = set([token2sents[i][a0],token2sents[i][b0],token2sents[i][c0]])
                if x == {0}:
                    continue
                x.discard(0)
                if x not in link:
            ...
            for k in range(len(link)):
                ...
                for j in range(k, len(link)):
                    if k==j:
                        continue
                    if link[k] <= link[j] or link[k] & link[j]:
                        link[j] = link[k] | link[j]
                        ff = 1
                        break
                    if link[j] <= link[k]:
                        link[j] = link[k]
                        ff = 1
                        break
                if ff==0 and link[k] not in pass_link:
                    pass_link.append(link[k])
            links[i] = pass_link
        # print(links, token2sents[0][-1])
        ...
        res = {
            ...
            'reply_masks': new_reply_masks, 'speaker_masks': new_speaker_masks,
            'thread_masks': new_thread_masks,
            'pieces2words': pieces2words, 'new2old': new2old,
            'hgraphs': hgraphs, 'utterance_spans': utterance_spans,
            #'input_ids2': input_ids2, 'input_masks2': input_masks2, 'input_segments2': input_segments2,
            'pred_t': pred_t, 'pred_a': pred_a, 'pred_o': pred_o,'links':links,'rel_lists':rel_lists,
        }

        nocuda = ['thread_lengths', 'pairs', 'doc_id', 'pieces2words', 'new2old', 'hgraphs', 'pred_t', 'pred_a', 'pred_o', 'links', 'rel_lists', 'utterance_spans']
...
class RelationMetric:
    ...
    def compute(self, name='valid'):
        ...
        micro, iden, res, intra, inter, pair, entiy = Run_eval(args).forward()
        self.clear()
        return micro[2], iden[2], res, intra[2], inter[2], pair, entiy
